...
export const VotingSection = ({ initialRollNumber = '', setActiveTab }) => {
  ...
  // Fetch student existing votes if roll is entered
  useEffect(() => {
      ...
      api.getVoterHistory(cleanRoll)
        .then(res => {
          if (res.success && res.data && Object.keys(res.data).length > 0) {
            setSelectedVotes(prev => ({ ...prev, ...res.data }));
          }
        })
        .catch(err => console.warn('Voter history note:', err));
    }
  }, [voterRoll]);
  ...
  const handleSubmitVotesAndStories = async (e) => {
    ...
    try {
      // 1. Submit Votes for each category
      if (hasVotes) {
        const votePromises = voteEntries.map(([catId, tId]) => {
          return api.voteTeacher(tId, cleanRoll, catId);
        });
        ...
        <div className="space-y-8">
          ...
          <div className="glass-card-glow rounded-3xl p-6 sm:p-10 border border-white/10 shadow-2xl space-y-6">
            ...
            <div className="max-w-xs relative">
              ...
              <input
                type="text"
                placeholder="Search CSE faculty..."
                value={teacherSearch}
              ...
              {filteredTeachers.map((teacher) => {
                ...
                return (
                  <div
                    ...
                    className={`p-3.5 rounded-2xl border cursor-pointer transition-all flex items-center justify-between gap-3 touch-press ${
          ...
          <div className="pt-2">
            <button
              ...
              disabled={loading}
              className="w-full py-4 sm:py-5 px-8 rounded-2xl bg-gradient-to-r from-purple-600 via-pink-600 to-amber-500 hover:from-purple-500 hover:to-pink-500 text-white font-black text-base sm:text-lg shadow-2xl shadow-purple-500/30 hover:scale-[1.01] transition-all flex items-center justify-center gap-3 touch-press"
            >
              <Send className="w-5 h-5" />
              <span>{loading ? 'Submitting...' : 'Submit Secret Votes & Anonymous Stories'}</span>
